...
export class MineBlockAtAction extends Action<MineBlockAtParams> {
    async do(possibleCheck: boolean = false, observation: Observation | undefined): Promise<ActionDoResult> {
        const block = this.bot.blockAt(this.options.position)
        ...
        if (block.harvestTools !== undefined) {
            try {
                assertHas(observation ?? await import("../Observer").then(({ observe }) => observe(this.bot)), 1,
        ...
        try {
            ...
            await moveToPositionWithRetry(this.bot, this.options.position)
            const target = this.bot.blockAt(this.options.position)
